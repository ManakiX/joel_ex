...
class User {
    ...
    async createUser(req, res) {
        // Payload
        let detail = req.body; //we are pulling everything within one file 
        // Hashing user password
        detail.userPass = await //user pass is the name of the column; when calling an async method, u need to use "await" (wait for the password)
        hash(detail.userPass, 15); //in JavaScript, the value is immutable (value can't be changed) has encrypts the password of the user. '15' is the salt value. tDO NOT USE WORKBENCH TO CREATE PASSWORD!!
